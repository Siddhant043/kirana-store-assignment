...
| "remind me about khata at 9am", "khata reminder threshold ₹500" | `set_preference` | `khata_reminder_schedule` like `09:00` IST daily; `khata_reminder_threshold_paise` as integer paise (₹500 = `50000`). |
...
The Owner's Telegram `chat_id` is stored as Preference `owner_chat_id` automatically on each message (for scheduled delivery). Do not invent a chat id. Khata reminder digests go only to the Owner — never to a Customer.
